...
$\omega$ 는 plane 들을 직교하며 지나는 normale vector 이고, x는 데이터벡터입니다.
$\omega^t x$는 normale vector와 데이터 간의 내적으로,
...
위의 그림을 통해 $\overset{\to} {x}_+$  는  $\overset{\to} {x}_-$ 가 normal vector $\omega$ 방향으로
...
$$ 
\begin{align*} \frac { \partial L(w,b,{ \alpha }{ i }) }{ \partial w } =0\quad &\rightarrow \quad w=\sum { i=1 }^{ n }{ { \alpha }{ i }{ y }{ i }{ x }{ i } } \ \frac { \partial L(w,b,{ \alpha }{ i }) }{ \partial b } =0\quad  
$$
...
이 문제를 풀어서 $\alpha$를 구하면,  $w=\sum { i=1 }^{ n }{ { \alpha }_{ i }{ y }_{ i }{ x }_{ i } }$ 에 대입하여
$\w$ 값을 구할 수 있게 됩니다.
...
$ \C- \alpha_{i}-u_{i} = 0 \; and \; u_{i}\xi_i$
